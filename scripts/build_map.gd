...
func _build() -> void:
	...
	# Furniture props - placed HERE at build time and saved into main.tscn
	# (moving a prop = edit PROP_PLACES below + re-run the builder).
	_props()
...
# ---------------------------------------------------------------- props
#
# Furniture, placed at BUILD time from the pre-baked scenes in
# assets/props/baked/ (made once by tools/prop_baker.tscn from the glbs).
# Each prop is auto-scaled and auto-centered, feet at the y you give.
# Solid props get a box collider on layer 1: the player bumps into them AND
# the navmesh carves around them so the entity walks around, never through.
# Colliders are shrunk a little (the shrink number) so a prop can never
# pinch a walkway shut and snag the entity.
#
# name -> [scale, solid, collider shrink]  (scales measured by prop_probe)
const PROP_DEFS := {
	"table":        [0.22, true, 0.9],
	"chair":        [1.0, true, 0.65],   # bench; backrest on -X, faces +X at yaw 0
	"bed_a":        [0.0106, true, 0.95],   # giant file -> ~2 m long (runs along X)
	"bed_b":        [1.15, true, 0.95],     # -> ~2 m long (runs along Z)
	"cupboard":     [1.0, true, 0.95],
	"trolley":      [1.0, true, 0.9],
	"lab_trolley":  [1.0, true, 0.35],      # bounding box is inflated, small collider
	"wall_cabinet": [1.2, false, 1.0],      # wall mounted, no collision
	"radiator":     [0.009, true, 0.9],     # giant file -> ~0.64 m wide
	"saline":       [0.207, true, 0.55],    # -> 1.8 m IV stand
}

# [name, Vector3(x, feet_y, z), yaw degrees]
# feet_y: 0 = floor 1, 4.1 = floor 2, 1.3/5.4 = wall-cabinet mount height.
const PROP_PLACES := [
	# ------------------------------------------------------------- FLOOR 1
	# Dining Room (x -55..-40)
	...
	["chair", Vector3(-46.5, 0, -30.2), -85.0],
	...
	# Lobby (x -21..3) - bench rows facing the reception counter, aisle at
	# the player spawn (x -11). Bench faces +X at yaw 0, so yaw 90 = north.
	["chair", Vector3(-17.0, 0, -33.3), 90.0],
	["chair", Vector3(-14.8, 0, -33.3), 94.0],
	["chair", Vector3(-7.5, 0, -33.3), 90.0],
	["chair", Vector3(-5.3, 0, -33.3), 86.0],
	["chair", Vector3(-17.3, 0, -35.5), 90.0],
	["chair", Vector3(-15.2, 0, -35.5), 92.0],
	["chair", Vector3(-6.7, 0, -35.5), 88.0],
	["chair", Vector3(-4.6, 0, -35.5), 90.0],
	["saline", Vector3(-16, 0, -29), 0.0],
	# Chapel (x 3..12) - pews facing the altar table on the north wall
	["chair", Vector3(5.7, 0, -34.4), 90.0],
	["chair", Vector3(8.2, 0, -34.4), 88.0],
	["chair", Vector3(5.7, 0, -32.4), 90.0],
	["chair", Vector3(8.2, 0, -32.4), 92.0],
	["table", Vector3(7.5, 0, -36.5), 0.0],
	# Waiting Area (x 12..30) - rows facing the corridor opening (south)
	["chair", Vector3(16, 0, -34), -90.0],
	["chair", Vector3(18.2, 0, -34), -90.0],
	["chair", Vector3(20.4, 0, -34), -92.0],
	["chair", Vector3(16, 0, -31.5), -90.0],
	["chair", Vector3(18.2, 0, -31.5), -88.0],
	["chair", Vector3(20.4, 0, -31.5), -90.0],
	["radiator", Vector3(29.7, 0, -33), 90.0],
	["trolley", Vector3(25, 0, -29.5), 45.0],
	# ER Registration (x 30..40)
	["chair", Vector3(32, 0, -34.5), -53.0],
	["wall_cabinet", Vector3(37, 1.3, -37.22), 0.0],
	["radiator", Vector3(30.25, 0, -31), 90.0],
	# ER Waiting (x 40..55) - columns facing the registration door (west)
	["chair", Vector3(44, 0, -35), 180.0],
	["chair", Vector3(44, 0, -33), 180.0],
	["chair", Vector3(44, 0, -31), -176.0],
	["chair", Vector3(46.5, 0, -35), 180.0],
	["chair", Vector3(46.5, 0, -33), -178.0],
	...
	["radiator", Vector3(-54.75, 0, -20), 90.0],
	# Offices (x -35..-25 and -25..-15)
	["table", Vector3(-30, 0, -20), 0.0],
	["chair", Vector3(-30.8, 0, -18.4), 63.0],
	["cupboard", Vector3(-28, 0, -24.5), 90.0],
	["radiator", Vector3(-32, 0, -14.25), 180.0],
	["table", Vector3(-20, 0, -19), 90.0],
	["chair", Vector3(-19, 0, -20.6), -122.0],
	...
	["radiator", Vector3(52, 0, -24.75), 0.0],
	# Physician Dining (x -55..-38, z -11..3)
	["table", Vector3(-48, 0, -6), 0.0],
	["table", Vector3(-43, 0, -2), 15.0],
	["chair", Vector3(-46.6, 0, -6), 180.0],
	["chair", Vector3(-49.6, 0, -4.5), 43.0],
	...
	["radiator", Vector3(-31, 0, -4.25), 180.0],
	# Dark Room (x -18..-10)
	...
	["radiator", Vector3(11, 0, 6.25), 0.0],
	...
	["radiator", Vector3(14, 0, 37.25), 180.0],
	# Utility room (x 24..40)
	["table", Vector3(30, 0, 28), 0.0],
	["chair", Vector3(31.5, 0, 29), 146.0],
	...
	["radiator", Vector3(54.75, 0, 34), -90.0],
	# Extra radiators, floor 1
	["radiator", Vector3(-36, 0, -37.25), 0.0],       # gift shop
	["radiator", Vector3(-20.75, 0, -31), 90.0],      # lobby, west wall
	["radiator", Vector3(5, 0, -37.25), 0.0],         # chapel
	# More furniture, floor 1 (July 2026 second pass)
	["table", Vector3(-52, 0, -30), 95.0],            # dining, NW corner
	["cupboard", Vector3(-33, 0, -37), 90.0],         # gift shop, north wall
	["cupboard", Vector3(-30.5, 0, -28.5), 90.0],     # restrooms, south wall
	["table", Vector3(26, 0, -33), 5.0],              # waiting area, east side
	["table", Vector3(50, 0, -30), 85.0],             # ER waiting
	["cupboard", Vector3(-50, 0, -24.5), 90.0],       # serving area, north wall
	["cupboard", Vector3(-48.6, 0, -24.5), 90.0],
	["cupboard", Vector3(-25.5, 0, -20), 180.0],      # office 1, east wall
	["cupboard", Vector3(23.5, 0, -22), 0.0],         # lab, east wall
	["cupboard", Vector3(31.5, 0, -22.5), 180.0],     # EKG, east wall
	["cupboard", Vector3(-10.5, 0, -9), 180.0],       # dark room, east wall
	["cupboard", Vector3(-54.5, 0, -8), 0.0],         # physician dining, west wall
	["bed_a", Vector3(16, 0, 28.5), -15.0],           # morgue, third slab
	["cupboard", Vector3(5.5, 0, 37), 90.0],          # central stores, row extension
	["cupboard", Vector3(-5.3, 0, 37), 92.0],
	["cupboard", Vector3(40.5, 0, 26), 180.0],        # ambulance bay, west wall
	# Floor-1 corridor dressing (close to walls, away from page spots)
	["trolley", Vector3(-5, 0, -27.5), 100.0],
	["chair", Vector3(30, 0, -25.7), -10.0],
	["saline", Vector3(-30, 0, -11.6), 0.0],
	["trolley", Vector3(10, 0, -13.5), -80.0],
	["bed_b", Vector3(38, 0, -12.8), 110.0],
	["trolley", Vector3(-33, 0, 4), 20.0],
	["chair", Vector3(12, 0, 5.2), 40.0],
	...
	# ------------------------------------------------------------- FLOOR 2
	# North patient rooms (beds against the north wall)
	...
	["radiator", Vector3(-16, 4.1, -37.25), 0.0],
	["radiator", Vector3(16, 4.1, -37.25), 0.0],
	["radiator", Vector3(38, 4.1, -37.25), 0.0],
	# Nurse station (counters at +-8.25, -19.5)
	["chair", Vector3(-8, 4.1, -21), -99.0],
	["trolley", Vector3(2, 4.1, -16), -45.0],
	["wall_cabinet", Vector3(14.72, 5.4, -18), -90.0],
	# Day Room (x -35..-15, z -25..-14)
	["table", Vector3(-28, 4.1, -19), 0.0],
	["chair", Vector3(-26.5, 4.1, -19.5), -162.0],
	["chair", Vector3(-29.5, 4.1, -18), 34.0],
	["table", Vector3(-22, 4.1, -21), 85.0],
	["chair", Vector3(-21, 4.1, -19.5), 124.0],
	...
	["radiator", Vector3(15.25, 4.1, -18), 90.0],
	...
	# (row kept clear of the door at x 28 on the z -11 wall)
	["cupboard", Vector3(24.9, 4.1, -10.52), 90.0],
	["cupboard", Vector3(26.2, 4.1, -10.52), 90.0],
	["cupboard", Vector3(31.5, 4.1, 1), 180.0],
	["wall_cabinet", Vector3(31.72, 5.4, -6), -90.0],
	["table", Vector3(28, 4.1, -1), 90.0],
	["radiator", Vector3(24.25, 4.1, -4), 90.0],
	...
	["radiator", Vector3(54.75, 4.1, 21), -90.0],
	# South patient rooms (beds against the south wall)
	["bed_b", Vector3(-24, 4.1, 36.9), 0.0],
	["saline", Vector3(-22.7, 4.1, 35.8), 0.0],
	["bed_b", Vector3(-16, 4.1, 36.9), 2.0],
	["chair", Vector3(-11, 4.1, 34), -10.0],
	...
	["radiator", Vector3(10, 4.1, 37.25), 180.0],
	["radiator", Vector3(41, 4.1, 37.25), 180.0],
	# More furniture, floor 2 (July 2026 second pass) - every north/south
	# patient room now has a bed, plus cupboards and tables spread around.
	["cupboard", Vector3(-44.5, 4.1, -31), 0.0],      # north room 1, east wall
	["table", Vector3(-36, 4.1, -32), 10.0],          # north room 2
	["bed_b", Vector3(-17.2, 4.1, -36.9), 181.0],     # north room 4 (was empty)
	["cupboard", Vector3(-11.5, 4.1, -33), 0.0],
	["bed_b", Vector3(13, 4.1, -36.9), 179.0],        # north room 7 (was empty)
	["cupboard", Vector3(32.5, 4.1, -31), 180.0],     # north room 8, east wall
	["table", Vector3(24, 4.1, -33), -8.0],
	["bed_b", Vector3(35, 4.1, -36.9), 178.0],        # north room 9 (was empty)
	["cupboard", Vector3(43.5, 4.1, -31), 0.0],
	["cupboard", Vector3(54.5, 4.1, -31), 0.0],       # north room 10, east wall
	["cupboard", Vector3(13, 4.1, -14.5), 90.0],      # nurse station, south wall
	["table", Vector3(-4, 4.1, -16), 3.0],            # nurse station, loose table
	["cupboard", Vector3(-15.5, 4.1, -20), 180.0],    # day room, east wall
	["table", Vector3(-25, 4.1, -16), -5.0],          # day room, third table
	["bed_b", Vector3(22, 4.1, -14.62), 2.0],         # treatment, second bed
	["cupboard", Vector3(15.5, 4.1, -22), 0.0],       # treatment, west wall
	["cupboard", Vector3(36.5, 4.1, -24.5), 90.0],    # east room, north wall
	["bed_b", Vector3(53.6, 4.1, -16), -90.0],        # far east room
	["cupboard", Vector3(-17.5, 4.1, -8), 0.0],       # OT 1, west wall
	["table", Vector3(20, 4.1, -8), -88.0],           # scrub/sterile
	["bed_b", Vector3(50, 4.1, -8), 90.0],            # ICU, third bed
	["table", Vector3(-24, 4.1, 17), 85.0],           # physio hall
	["cupboard", Vector3(-14.5, 4.1, 10), 180.0],     # physio, east wall
	["cupboard", Vector3(49, 4.1, 6.5), 90.0],        # iso ward 1, north wall
	["bed_b", Vector3(52, 4.1, 12.5), -88.0],         # iso ward 1, second bed
	["bed_b", Vector3(-36, 4.1, 36.9), 2.0],          # south room (was empty)
	["saline", Vector3(-34.7, 4.1, 35.8), 0.0],
	["bed_b", Vector3(6, 4.1, 36.9), 179.0],          # south room (was empty)
	["cupboard", Vector3(3.5, 4.1, 33), 0.0],
	["cupboard", Vector3(14.5, 4.1, 35), 0.0],        # south room, west wall
	["bed_b", Vector3(39, 4.1, 36.9), -179.0],        # south room (was empty)
	["cupboard", Vector3(46.5, 4.1, 33), 180.0],
	# Floor-2 corridor dressing
	["bed_b", Vector3(38, 4.1, -12.8), 95.0],
	["trolley", Vector3(5, 4.1, -13.4), 70.0],
	["chair", Vector3(-10, 4.1, -27.6), -160.0],
	["saline", Vector3(15, 4.1, -25.7), 0.0],
	["trolley", Vector3(-25, 4.1, 21.5), -30.0],
]

# One measured template per prop: { packed, size, center, min_y }
var prop_lib := {}


func _props() -> void:
	var count := 0
	for p in PROP_PLACES:
		if _place_prop(p[0], p[1], deg_to_rad(p[2])):
			count += 1
	print("PROPS: ", count, " placed into the scene")


# Loads one baked prop scene and measures its real size (once per prop).
func _prop_template(key: String) -> Dictionary:
	if prop_lib.has(key):
		return prop_lib[key]
	var packed: PackedScene = load("res://assets/props/baked/" + key + ".scn")
	if packed == null:
		push_warning("PROPS: missing baked scene for " + key + " - run tools/prop_baker.tscn")
		prop_lib[key] = {}
		return {}
	var inst := packed.instantiate()
	var out: Array = []
	_measure(inst, Transform3D.IDENTITY, out)
	inst.free()
	if out.is_empty():
		push_warning("PROPS: no meshes in " + key)
		prop_lib[key] = {}
		return {}
	var ab: AABB = out[0]
	prop_lib[key] = {"packed": packed, "size": ab.size, "center": ab.get_center(), "min_y": ab.position.y}
	return prop_lib[key]


# Walks a prop's nodes adding up transforms (the node isn't in the tree, so
# global_transform doesn't work) and grows one box around all its meshes.
func _measure(n: Node, xf: Transform3D, out: Array) -> void:
	if n is Node3D:
	...
	if n is MeshInstance3D and (n as MeshInstance3D).mesh != null:
		...
		if out.is_empty():
			...
			out[0] = out[0].merge(ab)
	for c in n.get_children():
		_measure(c, xf, out)


# One prop: centered on x/z, feet at pos.y, box collider if solid.
func _place_prop(key: String, pos: Vector3, yaw: float) -> bool:
	var t := _prop_template(key)
	if t.is_empty():
		return false
	var def: Array = PROP_DEFS[key]
	var s: float = def[0]

	var prop_root := Node3D.new()
	prop_root.name = nn("Prop")
	prop_root.position = pos
	prop_root.rotation.y = yaw

	var model: Node3D = t.packed.instantiate()
	model.name = "Model"
	model.scale = Vector3.ONE * s
	model.position = Vector3(-t.center.x * s, -t.min_y * s, -t.center.z * s)
	prop_root.add_child(model)

	if def[1]:  # solid: collider on layer 1 (blocks player, carves navmesh)
		var body := StaticBody3D.new()
		body.name = "Solid"
		var cs := CollisionShape3D.new()
		cs.name = "Shape"
		var shrink: float = def[2]
		cs.shape = _shape(Vector3(t.size.x * s * shrink, t.size.y * s, t.size.z * s * shrink))
		cs.position = Vector3(0, t.size.y * s * 0.5, 0)
		body.add_child(cs)
		prop_root.add_child(body)

	nav.add_child(prop_root)
	return true
